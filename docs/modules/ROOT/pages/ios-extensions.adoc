...
Teak uses link:https://developer.apple.com/documentation/usernotifications/modifying-content-in-newly-delivered-notifications?language=objc[iOS Notification Extensions,window=_blank] to

* Display rich media such as images, video, and custom thumbnails in iOS push notifications
* Track confirmed receipts of iOS push notifications
* Track impressions of rich media content
* Display action buttons on expanded iOS push notifications

We recommend all games integrate Teak's iOS Notification Extensions for maximum functionality.

If you have xref:quickstart/install-sdk.adoc#_configure_teak_in_the_unity_editor[Build Post-Processing,window=_blank] enabled then the Teak SDK's included Build Post-Processing scripts will add the Teak iOS Notification Extensions to your Xcode project and build automatically. You're done and can stop reading this page :)
...
If you are not using Teak's Build Post-Processing scripts then you will need to manually add the Teak iOS Notification Extensions to your project. We recommend updating your own build scripts to manage this process.
...
* `TeakExtension.xcframework`

image::service-extension-link.png[]

NOTE: You can find `TeakExtension.xcframework` in `Frameworks/Teak/Plugins/iOS` in the Xcode project
...
// is in 'TeakExtension.xcframework'. It is done this way so you do not need to
// fight with include paths.
...
in the functionality from `TeakExtension.xcframework` which will take care of everything else.
...
The Teak SDK includes a PrivacyInfo.xcprivacy suitable for the notification service extension in `Assets/Teak/Editor/iOS/TeakNotificationService/PrivacyInfo.xcprivacy`. This file or a similar one should be added to the `Copy Bundle Resources` build phase of the `TeakNotificationService` target.
...
NOTE: You can find `TeakExtension.xcframework` in `Frameworks/Teak/Plugins/iOS` in the Xcode
