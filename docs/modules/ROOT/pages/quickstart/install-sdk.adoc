...
* [x] Teak supports `Unity {min-unity}` or newer.
* [x] On iOS, Teak requires iOS {min-ios}+ and Xcode {min-xcode}+.
* [x] On Android, Teak requires Android {min-android-name} (API level {min-android-sdk}) or newer.
...
----
+
TIP: For reproducible builds, pin to a specific version by appending `#<version>` to the URL, e.g. `\https://github.com/GoCarrot/upm-package-teak.git#4.3.10`. See xref:upm.adoc[Unity Package Manager] for all version pinning options.
...
+
[NOTE]
If your project's scripts live in their own **Assembly Definition** files, add a reference to the `Teak` assembly (and `Teak.Editor` if your editor scripts call Teak's editor APIs) from each assembly definition that uses Teak — otherwise you'll hit `The type or namespace name 'Teak' could not be found` compile errors. See xref:upm.adoc#_assembly_definitions[Assembly Definitions] for the Inspector and `.asmdef` steps.
