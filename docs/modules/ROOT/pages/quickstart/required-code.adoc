...
Get your code editor ready, we need to do two things: identify the user, and ask for push permissions.

== Identify User

We tell Teak how to identify the current user by sending the player's ID with `<<IdentifyUser(string,UserConfiguration)>>[IdentifyUser()]`. 
